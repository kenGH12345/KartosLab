...
class _MagicLabScreenState extends State<MagicLabScreen>
  ...
  void initState() {
    ...
    _bubbleTicker = createTicker((_) => setState(() => _bubblePhase += 0.05))
      ..start();
  ...
  void _startChallenge() {
    ...
    _state.updateIntensity(0, 0);
    _state.updateIntensity(1, 0);
    _state.updateIntensity(2, 0);
    _timeLeft = _challengeTimeLimit;
  ...
  double _calcAccuracy() {
    ...
    final dr = (t.r - cur.r).abs(),
        dg = (t.g - cur.g).abs(),
        db = (t.b - cur.b).abs();
    final acc = ((1 - sqrt(dr * dr + dg * dg + db * db) / sqrt(3.0)) * 100)
        .clamp(0.0, 100.0);
  ...
  void _onWheelTap(TapDownDetails d) {
    final dx = d.localPosition.dx - _wheelCx,
        dy = d.localPosition.dy - _wheelCy;
    if (sqrt(dx * dx + dy * dy) > _wheelR) return;
    double hue = (atan2(dy, dx) * 180 / pi + 360) % 360;
    setState(() {
      _wheelPoint = d.localPosition;
      _wheelColor = HSVColor.fromAHSV(
        1.0,
        hue,
        (sqrt(dx * dx + dy * dy) / _wheelR).clamp(0.0, 1.0),
        _wheelBrightness,
      ).toColor();
    });
  }
  ...
  void _switchMode(LabMode m) {
    if (m == LabMode.challenge && _cfg == null) {
      debugPrint(
        'DEPRECATED: challenge config missing for ${_scenario?.scenarioId}, using hardcoded fallback',
      );
    }
    setState(() {
      _mode = m;
      if (m == LabMode.challenge) {
        _startChallenge();
      } else {
        _challengeActive = false;
        _timerTicker?.stop();
    ...
    return Stack(
      children: [
        NineGridLayout(
          ...
          topRight: widget.scenarioList.isNotEmpty
              ? _buildScenarioMenu()
              : null,
          ...
          // 底部横条 = 模式相关操作面板横排（molarity footer 方案推广）
          footer: _buildFooterControls(),
          // 底部中格 = 实验说明 + 操作指引（通用引导组件）
          bottomCenter: ExperimentIntroPanel(
  ...
  Widget _buildScenarioMenu() {
    ...
    return ScenarioMenuButton(
      ...
      onSelected: (id) {
        final next = widget.scenarioList
  ...
  Widget _buildCanvas() {
    ...
    if (_mode == LabMode.wheel) {
      return LayoutBuilder(
        builder: (ctx, c) {
          _wheelCx = c.maxWidth / 2;
          _wheelCy = c.maxHeight * 0.38;
          _wheelR = min(_wheelCx, _wheelCy) * 0.68;
          return GestureDetector(
            onTapDown: _onWheelTap,
            child: CustomPaint(
              size: Size.infinite,
              painter: ColorWheelPainter(
                brightness: _wheelBrightness,
                selectedPoint: _wheelPoint,
                selectedColor: _wheelPoint != null ? _wheelColor : null,
                wheelCenterX: _wheelCx,
                wheelCenterY: _wheelCy,
                wheelRadius: _wheelR,
              ),
            ),
          );
          ...
          return GestureDetector(
            ...
            child: CustomPaint(
              ...
              painter: ChallengePainter(
                ...
                currentColor: _state.mixedColor,
                accuracy: _calcAccuracy(),
                score: _score,
                streak: _streak,
                level: _level,
                timeLeft: _timeLeft,
                showTarget: true,
                redIntensity: _state.redIntensity,
                greenIntensity: _state.greenIntensity,
                blueIntensity: _state.blueIntensity,
              ),
            ),
          );
        },
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  gradient: RadialGradient(
                    colors: [mixed.withAlpha(30), Colors.transparent],
                    radius: 0.8,
                  ),
                ),
                child: Center(
                  // FittedBox 缩放：320 窄视口主画面高度不足时混合色大圆整体缩放（77px 溢出修复）
                  child: FittedBox(
  ...
  /// 底部横排操作面板（模式驱动 · molarity footer 方案推广 · 横向滚动兜底窄视口）。
  Widget _buildFooterControls() {
    return SingleChildScrollView(
      scrollDirection: Axis.horizontal,
      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4),
      child: Row(children: _footerModeControls()),
    );
  }

  List<Widget> _footerModeControls() {
    if (_mode == LabMode.wheel) {
      return [
        SizedBox(
          width: 220,
        ...
        _swatchColumn(_wheelColor, _nameColor(_wheelColor), box: 24),
      ];
    }
    if (_mode == LabMode.challenge) {
      return [
              ...
              if (!_challengeActive && _timeLeft <= 0) ...[
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: const Text('重新开始', style: TextStyle(fontSize: 10)),
  ...
  /// 色块 + 颜色名（wheel / explore 共用）。
  Widget _swatchColumn(Color color, String name, {double box = 24}) {
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        Container(
  ...
  /// 挑战模式进度条 + 准确率（footer 用 · 原 _modeControls 已删除，逻辑内联至此）。
  Widget _challengeProgressRow() {
          ...
          child: SizedBox(
            ...
            child: ClipRRect(
              ...
              child: LinearProgressIndicator(
                value: (_timeLeft / totalTime).clamp(0.0, 1.0),
  ...
  Widget _miniSliderVertical(int ch, String label, Color color) {
    final v = ch == 0
        ? _state.redIntensity
        : ch == 1
        ? _state.greenIntensity
        : _state.blueIntensity;
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text(
        ...
        KnowledgeSection.list(
          ...
          items: const [
            ...
            KnowledgeItem(
              ...
              titleColor: Color(0xFF22C55E),
              desc:
                  '人眼视网膜有三种视锥细胞：L(长波/红)、M(中波/绿)、S(短波/蓝)。大脑通过比较三种细胞的响应强度来感知颜色。'
            ...
            KnowledgeItem(
              ...
              desc:
                  '加色法(光源RGB): 越混越亮,三原色全开=白光。减色法(颜料CMYK): 越混越暗,三原色全混=黑色。'
            ...
            KnowledgeItem(
              ...
              desc:
                  'RGB能覆盖的色彩范围叫"色域"。不同显示设备（手机OLED/液晶/投影仪）的色域不同——sRGB是最通用的标准色域。'
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: active ? color : color.withAlpha(100),
            width: 1.5,
          ),
        ),
        child: Text(
