...
class _NetForceScreenState extends State<NetForceScreen>
      ...
      child: Stack(
        children: [
          NineGridLayout(
            ...
            // 右下格 = 实验说明 + 操作指引（通用引导组件）
            bottomRight: ExperimentIntroPanel(
  ...
  /// 右侧边格控制面板 · 竖排紧凑 · 窄条可滚动
  /// 底部横排操作面板：显示 chips + 时间控制 + 归位 + 总力记分板
  ...
  Widget _buildSidePanel() {
    return SingleChildScrollView(
      ...
      child: FittedBox(
        ...
        child: Row(
          children: [
            // Wrap 需限定宽度：FittedBox 在横向滚动内给 child 无界宽，Wrap 无限展开会溢出
            SizedBox(
              width: 200,
              child: Wrap(
                spacing: 6,
  ...
  Widget _buildForceDisplay() => LayoutBuilder(
    builder: (ctx, c) {
      ...
      return Stack(
        children: [
              ...
              child: SizedBox(
                ...
                child: CustomPaint(
                  painter: ForceArrowPainter(
                    magnitude: _model.leftForce,
                    direction: false,
                    color: const Color(0xFF3B82F6),
                    label: _showValues
                        ? _model.leftForce.toStringAsFixed(0)
                        : null,
                    maxForce: 350,
                    centered: true,
          ...
          if (_showSum && _model.rightPullers.isNotEmpty)
            Positioned(
              ...
              child: SizedBox(
                ...
                child: CustomPaint(
                  painter: ForceArrowPainter(
                    magnitude: _model.rightForce,
                    direction: true,
                    color: const Color(0xFFEF4444),
                    label: _showValues
                        ? _model.rightForce.toStringAsFixed(0)
                        : null,
                    maxForce: 350,
                    centered: true,
                    labelAlign: LabelAlign.center,
          ...
          if (_showSum)
            Positioned(
              ...
              child: SizedBox(
                ...
                child: CustomPaint(
                  painter: ForceArrowPainter(
                    magnitude: _model.netForce.abs(),
                    direction: _model.netForce >= 0,
                    color: const Color(0xFF7C3AED),
                    label: _showValues
                        ? 'Σ${_model.netForce.toStringAsFixed(0)}'
                        : null,
                    maxForce: 350,
                    centered: true,
                    labelAlign: LabelAlign.center,
          ...
          ...List.generate(
            4,
            (i) =>
                _knotDragTarget(cartX - 80 - (i + 1) * 60.0, cy + 90, false, i),
          ),
          ...List.generate(
            4,
            (i) => _knotDragTarget(cartX + 80 + i * 60.0, cy + 90, true, i),
          ),
          ...
          if (!_model.isRunning && !_model.isGameOver)
            Positioned(left: cx - 60, top: cy + 4, child: _buildStartButton()),
        ],
      );
    },
  );
  ...
  Widget _knotDragTarget(double left, double top, bool side, int idx) {
    ...
    final hasPuller = _model.pullers.any(
      (p) => p.side == side && p.knotIndex == idx,
    );
    return Positioned(
      left: left - 10,
      top: top - 10,
  ...
  Widget _pullerTray(bool side) => Wrap(
    children: _model.pullers
        .where((p) => p.side == side && p.knotIndex == null)
        .map(
          (p) => Draggable<Puller>(
            data: p,
            feedback: Material(
        ...
        KnowledgeSection.list(
          ...
          items: const [
            KnowledgeItem(
              ...
              titleColor: Color(0xFFF59E0B),
              desc:
                  '拔河的胜负取决于合力方向。合力向右→车向右加速；合力向左→车向左加速。'
            ...
            KnowledgeItem(
              ...
              titleColor: Color(0xFF22C55E),
              desc:
                  '拔河中双方对绳子的拉力大小永远相等(牛顿第三定律)。那为什么会有胜负?因为'
            ...
            KnowledgeItem(
              ...
              desc:
                  '桥梁、建筑、塔吊——所有结构设计的核心都是"让合力为零"。每个构件受到的所有力(重力/拉力/压力/支撑力)'
