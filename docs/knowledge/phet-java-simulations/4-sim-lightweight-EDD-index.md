...
> **数据源**：`c:\workspace\phetTrunk\phetTrunk\simulations-java\simulations\<sim>\src\edu\colorado\phet\<pkg>\` 实测扫描
...
| **业务代码规模** | 33 | ~50（含 model/view/coreadditions） | ~40（不算 common_1200 内嵌） | ~120（不算 tests/ 与 phetcommon） |
| **子屏数量（Module）** | 2 | 5 | 1 | 3 |
...
├── common_1200/graphics/  (17 · 内嵌 phet-common · 复刻时用 Flutter 原生 widget)
...
- ⚠️ **common_1200/graphics/** 是 phet 早期公共库快照，Flutter 版用原生 widget 全部替代
...
### 目录结构（核心业务代码）
```
edu/colorado/phet/waveinterference/
...
- 产物：`context/project/phet-java-simulations/edd/color-vision-EDD.md`（预计 400-600 行）
...
> - 数据源：实测 `c:\workspace\phetTrunk\phetTrunk\simulations-java\simulations\<sim>\src\` 4 目录
