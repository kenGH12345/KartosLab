...
> 目的（关系 c3）：帮助 Flutter phet 复刻者快速找到"当前已有 3 个模块的 Java 参考实现在哪、哪些功能可以从 Java 版借鉴补齐"
> 数据源：[Flutter 侧 systems/module-index.md](../phet/systems/module-index.md) × [phet-java catalog](module-catalog.md)
> 分析日期：2026-07-22
...
- **Java 源码位置**：`<PHET_JAVA_ROOT>/simulations-java/simulations/circuit-construction-kit/src/edu/colorado/phet/circuitconstructionkit/`
- **规模**：172 Java 文件 · 依赖 `jfreechart-phet + Jama + nanoxml`（Jama 用于矩阵解方程）
- **Flutter 对应**：
  ...
  - 详见 [phet/systems/circuit-module.md](../phet/systems/circuit-module.md)

### 相关辅助 sim（可借鉴功能）

| Java sim | 可借鉴给 Flutter circuit 的部分 | Java 文件位置 |
|---|---|---|
| `battery-resistor-circuit` | 电子小球动画（Flutter 已删除的 ElectronAnimator 可参考 CCK 之外的极简版） | `simulations/battery-resistor-circuit/src/edu/colorado/phet/batteryresistorcircuit/` |
| `signal-circuit` | 信号脉冲传播可视化 | `simulations/signal-circuit/src/edu/colorado/phet/signalcircuit/` |
| `capacitor-lab` | 电容元件（Flutter 尚未支持） | `simulations/capacitor-lab/src/edu/colorado/phet/capacitorlab/` |
...
- Flutter 侧的 **场景 JSON + AI 生成 prompts/schemas**（详见 [phet/architecture/design-patterns.md](../phet/architecture/design-patterns.md)）是**领先 CCK 的原创能力**，复刻新功能时应同步补 JSON schema
...
- **Java 源码位置**：`<PHET_JAVA_ROOT>/simulations-java/simulations/bending-light/src/edu/colorado/phet/bendinglight/`
- **规模**：62 Java 文件 · 依赖 `piccolo-phet`（🟢 PC · 复刻门槛低）
...
- **Flutter 对应**：
  ...
  - 详见 [phet/systems/optics-module.md](../phet/systems/optics-module.md)

### 相关辅助 sim（可借鉴功能）

| Java sim | 可借鉴给 Flutter optics 的部分 | Java 文件位置 |
|---|---|---|
| `color-vision` | 光的颜色 / RGB 合成（Flutter optics 尚未涉及） | `simulations/color-vision/src/edu/colorado/phet/colorvision/` |
| `wave-interference` | 光波干涉（若扩展到"波动光学" · **196 Java 文件**大型 sim，慎抽） | `simulations/wave-interference/src/edu/colorado/phet/waveinterference/` |
| `fourier` | 傅里叶合成（波形叠加 · 与光学干涉相关） | `simulations/fourier/src/edu/colorado/phet/fourier/` |
...
- **Java 源码位置**：`<PHET_JAVA_ROOT>/simulations-java/simulations/forces-and-motion-basics/src/edu/colorado/phet/forcesandmotionbasics/`
- **规模**：28 Java 文件 · 依赖 `piccolo-phet`（🟢 PC · **最小最直接的蓝本**）
...
- **Flutter 对应**：
  ...
  - 详见 [phet/systems/forces-module.md](../phet/systems/forces-module.md)

### 相关辅助 sim（可借鉴功能）

| Java sim | 可借鉴给 Flutter forces 的部分 | Java 文件位置 |
|---|---|---|
| `forces-1d` | 一维力分析（Java 文件 65 · 更详细的 Motion 屏原型） | `simulations/forces-1d/src/edu/colorado/phet/forces1d/` |
| **`the-ramp`** | **斜坡场景（Flutter 尚未支持 · 强烈推荐补齐）** | `simulations/the-ramp/src/edu/colorado/phet/theramp/` |
| `moving-man` | 位置/速度/加速度 三图联动（Flutter forces 若加图表可参考） | `simulations/moving-man/src/edu/colorado/phet/movingman/` |
| `gravity-and-orbits` | 引力（若扩展二维万有引力） | `simulations/gravity-and-orbits/src/edu/colorado/phet/gravityandorbits/` |
| `energy-skate-park` | 能量守恒（若扩展"能量视角"） | `simulations/energy-skate-park/src/edu/colorado/phet/energyskatepark/` |

### Java → Flutter 复刻要点提示

- **forces-and-motion-basics 是 PhET 官方 Java 版里最"简洁"的力学 sim**（28 文件），非常适合 1:1 对照阅读
- **Flutter 已覆盖 Motion + NetForce 两屏**，最容易补齐的下一步是 **the-ramp 斜坡**（Java 文件 97 · 中等 · 🟠 依赖 phetgraphics 老栈需注意图形层需要重写）
...
1. **理解 Java 蓝本**：阅读 `<sim>/src/edu/colorado/phet/<sim>/` 目录顶层 `<Sim>Application.java` + `<Sim>Module.java` + `model/*.java`
2. **抽 MVC 三层**：
...
3. **补配置化层**（**Flutter 独有**）：
   ...
   - 详见 [phet/architecture/design-patterns.md](../phet/architecture/design-patterns.md) "配置化"节 与 [phet/architecture/ai-generation-readiness.md](../phet/architecture/ai-generation-readiness.md)
4. **遵守四原则合规**：MVC / 组件化 / 通用化 / 配置化（详见 [phet 侧四原则表](../phet/systems/module-index.md#设计原则对照总表速查)）
...
- Flutter 侧模块深度文档 → [phet/systems/](../phet/systems/)
