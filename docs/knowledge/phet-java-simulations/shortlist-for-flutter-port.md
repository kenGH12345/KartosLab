...
> 目的（关系 c1）：从 81 个有效 sim 中挑选**最适合 Flutter phet 项目下一步复刻**的候选，四维打分排序
...
> **重要**：Java 版 `common/*` 库是**所有 sim 共享的通用能力**，Flutter phet 现有 3 模块用到的只是冰山一角。补齐 Wave 0 会让后续 Wave A/B 的每个 sim 复刻工作量**批量减半**。
> 分析来源：[overview.md § 技术栈](overview.md) + Flutter phet [architecture/design-patterns.md](../phet/architecture/design-patterns.md) + [frontend/ui-framework.md](../phet/frontend/ui-framework.md)

### Flutter phet 已有的通用基础（勿重复造轮子）

| 已有能力 | Flutter 侧位置 | Java 蓝本对应 |
|---|---|---|
| `DragDropWorkspace<T>` 泛型拖拽画布 | `lib/common/widgets/drag_drop_workspace.dart` | `common/piccolo-phet` 的 `PhetPCanvas` |
| `CanvasProjection` / `SceneProjection` 世界↔屏幕坐标 | 同上 + `circuit_screen.dart:312` | `common/piccolo-phet` 的 `ModelViewTransform2D` |
| `ScenarioManager` JSON 场景加载（§C1 硬约束） | `lib/optics/config/scenario_manager.dart` | ✨ **Flutter 独有** · Java 版没有 |
| `CustomPainter` 规范（构造注入 + toScreen + shouldRepaint） | [conventions/add-custom-painter.md](../phet/conventions/add-custom-painter.md) | `common/piccolo-phet` 的 `PNode` 图形栈 |
| 不可变 + copyWith + Sentinel | `lib/models/circuit_state.dart` | ✨ **Flutter 独有** · Java 版可变 Model |
| `SoundEffects` 音效服务 | `lib/circuit/services/sound_effects.dart`（circuit 专属） | `common/phetcommon` 的 `SoundHandler` |
...
|---:|---|---|---|:---:|
| 🥇 W0-1 | **图表控件（Chart / 曲线图）** | `common/jfreechart-phet/` | moving-man · energy · natural-selection · ph-scale · 30+ sim | 🔴 P0 |
...
| 🥈 W0-3 | **PropertyControl（滑动条 / 数值输入 / 单选组）** | `common/piccolo-phet/` 的 `LinearValueControl` | ph-scale · molarity · capacitor-lab · 几乎所有 sim | 🔴 P0 |
| 🥈 W0-4 | **矢量场绘制（箭头场 / ArrowNode）** | `common/piccolo-phet/` 的 `ArrowNode` + `charges-and-fields` | efield · gravity-and-orbits · balloons(静电) · capacitor-lab | 🟠 P1 |
...
- **紧迫理由**：Java 版 `jfreechart-phet` 被 **30+ sim** 依赖；Flutter phet 目前 3 模块都是纯画布，没有一个 sim 出过图表
...
- **紧迫理由**：Java 版右侧控制面板全部是 `LinearValueControl` / `PropertySlider`；Flutter phet 每个 Screen 的 `_RightPanel` 手写 `Slider`+`TextField`（`optics_screen.dart` 就有）
...
- ⚠️ Wave 0 属于 **Flutter phet 项目的架构演进**，不属于 phet-java-simulations 知识库的核心职责（本知识库只是"蓝本地图"）
- 若采纳 Wave 0，建议在 [phet/architecture/](../phet/architecture/) 侧新建 `common-capabilities-roadmap.md`，本节仅作**索引和触发点**
- 每个 Wave 0 组件的详细设计（API / 测试 / 场景 JSON 集成）应由 Flutter phet 侧的需求驱动（走 `req-phet-*` 命名）
...
- 1 = 🟢 PC（仅 phetcommon + piccolo-phet）
- 2 = 🟡 PC+（+ jfreechart-phet / chemistry / games / motion 等标准辅助）
- 3 = 🟡 PC+ 带较冷门库（如 spline / timeseries / functionaljava）
- 4 = 🟠 老栈（含 phetgraphics 需重写图形层）或 jmol
- 5 = 🔴 3D（lwjgl）/ jbox2d
...
**理由**：Flutter phet 已有 circuit/optics/forces 的架构基础，补齐同模块内的功能屏收益最大，风险最小。

| sim | 补给 Flutter 哪个模块 | 工作量估计 |
|---|---|---|
| **the-ramp** | forces | 中（依赖 phetgraphics 老栈，需重写图形层，但物理简单） |
...
**理由**：如 Flutter phet 准备扩展学科覆盖，这些 sim 是"性价比最高的第一枪"。
...
- **国内高中教材匹配度**：Flutter phet 的原始定位是否面向国内 K-12，会改变教学价值权重
