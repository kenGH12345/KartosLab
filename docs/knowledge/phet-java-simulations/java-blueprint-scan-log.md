...
> **数据源**：`c:\workspace\phetTrunk\phetTrunk\simulations-java\simulations\<sim>\src\edu\colorado\phet\<pkg>\`

---

## 一、Java 蓝本磁盘路径（已实测确认 2026-07-24）

**PHET_JAVA_ROOT** = `c:\workspace\phetTrunk\phetTrunk\`（注意嵌套两层）

4 个目标 sim 完整路径：

| sim | 完整路径 |
|---|---|
| color-vision | `c:\workspace\phetTrunk\phetTrunk\simulations-java\simulations\color-vision\src\edu\colorado\phet\colorvision\` |
| sound | `c:\workspace\phetTrunk\phetTrunk\simulations-java\simulations\sound\src\edu\colorado\phet\sound\` |
| radio-waves | `c:\workspace\phetTrunk\phetTrunk\simulations-java\simulations\radio-waves\src\edu\colorado\phet\radiowaves\` |
| wave-interference | `c:\workspace\phetTrunk\phetTrunk\simulations-java\simulations\wave-interference\src\edu\colorado\phet\waveinterference\` |
...
**产出文件**：
- `context/project/phet-java-simulations/4-sim-lightweight-EDD-index.md`（4 sim 轻量 EDD 索引 · Q1-Q6 全景对照）
- `context/project/phet-java-simulations/edd/color-vision-EDD.md`（**首个完整 EDD · 8 章节全量 · 作为后续 sim 模板锚点**）
