...
> **蓝本**：`c:\workspace\phetTrunk\phetTrunk\simulations-java\simulations\radio-waves\` · 57 Java 文件
...
> **数据源**：`PHET_JAVA_ROOT/simulations-java/simulations/radio-waves/src/edu/colorado/phet/radiowaves/**/*.java`
...
| `EmfControlPanel` | ❌ 用 L0 `PhetSlider` / `PhetRadioGroup` / `PropertyControlPanel` |
...
| 组件 | L0 路径 | 必用/可选 |
|---|---|---|
| PhetSlider（频率/振幅） | `lib/common/controls/phet_slider.dart` | 必用 |
| PhetRadioGroup（正弦/手动 · 场/力 · 全矢量/单行） | `lib/common/controls/phet_radio_group.dart` | 必用 |
