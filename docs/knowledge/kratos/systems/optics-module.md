...
| 资产 | 位置 | 用途 |
|---|---|---|
| System Prompt | `C:\workspace\kratos\docs\prompts\optics_scenario.md`（277 行） | 教 LLM 光学 scenario 字段结构 + 6 种元件 + 3 类约束 + 3 类成功标准 |
| JSON Schema | `C:\workspace\kratos\schemas\optics_scenario.schema.json`（177 行） | 校验 LLM 输出结构合法性（Draft 2020-12） |
