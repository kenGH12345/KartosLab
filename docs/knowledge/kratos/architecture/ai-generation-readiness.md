...
>   2. Step 1 · `req-kartosos-circuit-config-json` · 电路配置化 · 2-4 天 · agile-vibe · ✅ **done 2026-07-21**
>   3. Step 2 · `req-kartosos-ai-scenario-toolchain-lite` · AI 工具链 B 模式（三模块）· 0.5-1 天 · agile-vibe · ✅ **done 2026-07-21**
>   4. Step 3 · `req-kartosos-forces-config-json` · 力与运动配置化 · 5-8 天 · agile-vibe · ✅ **done 2026-07-21**
> **artifacts**（工具链文件清单）:
...
本文档回答一个问题：**如果想让 AI（LLM）为 kartosos 项目生成新的实验场景，需要做哪些改造？**
...
```
c:\workspace\kratos\assets\scenarios\
...
- [ ] 知识库同步：`docs/knowledge/kkartoss/systems/circuit-module.md` 加"配置化"章节
...
| 粒度选项 | 说明 | 推荐 |
|---|---|---|
| **粒度 1（推荐）**：三个独立需求分批 | `req-kartosos-refactor-baseline`（方案 A 3.1+3.2）→ `rekartosatos-circuit-config-json`（本方案阶段 1）→ `kartoskratos-ai-scenario-gen`（本方案阶段 2）| ✅ 每个需求都能独立验收、独立回滚 |
| 粒度 2：一个大需求 | `req-kartosos-ai-generation-ready`（一次搞定方案 A + 方案 B 全部）| ❌ 3-6 天工作量过大，违反 10-vibecoding-protocol §"30 分钟原则"的精神（虽然协议原本针对 vibe-loop） |
| 粒度 3：只做阶段 1 | `req-kartosos-circuit-config-json` 单独立项 | ⚠️ 只有配置化没有 AI 生成，Q1 目标不完整 |
