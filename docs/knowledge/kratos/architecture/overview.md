...
> 来源: 首次扫描（`C:\workspace\kratos`）| 创建时间: 2026-07-17
> 变更: 2026-07-17 移除未接入主流程的 `ElectronAnimator`（架构图 L4 + 目录树 services/），总文件数 52 → 50。

`kratos` 是一个原生 Flutter 几何光学 / 电路 / 力学学习模拟应用（PhET 风格教学 App），语言为 **Dart**，运行目标包含 Web / Android / iOS / Windows / macOS / Linux。
...
| 入口 | `lib/main.dart` → `KratosApp`（MaterialApp）→ `HomeScreen` | `lib/main.dart:7-56` |
...
- 业务代码根: `C:\workspace\kratos\lib`
- 外部资产（SVG/场景 JSON）: `C:\workspace\kratos\assets`（场景配置在 `assets/scenarios/manifest.json` + 各 `<id>.json`，由 `ScenarioManager.loadScenarios` 加载）
