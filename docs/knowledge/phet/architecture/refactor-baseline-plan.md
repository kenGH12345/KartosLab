...
2026-07-20 下午，AI 会话按本文档 §3.1 执行了 `git rm -r lib/models/`（phet 项目 commit `df833b6`），随即跑 `flutter analyze` 冒烟验收，**发现 228 处编译 error**——因为 `lib/models/` 是**活代码基座**（被 `circuit_screen.dart` / 6 个 widgets / `test/widget_test.dart` 通过 `import '../models/*.dart'` 大量引用），不是死代码。

立即执行 `git revert df833b6` 回退（commit `7c3c1e3`），phet 项目 HEAD 恢复到功能上等价于 baseline `c2d47c2` 的状态，`flutter analyze` 回到 39 issues（全 warning，无 error）。

phet 项目 git 保留 `df833b6 → 7c3c1e3` 的"错误 + 回滚"痕迹作为教训物证，**不做 `git reset --hard` 抹除**。
...
1. **grep 作用域漏洞**：原调研只 grep `lib/` 目录且用模式 `models/$name`，漏了 `test/` 目录、漏了 `package:geometric_optics/models/xxx.dart` 完整包路径 import 形式。
...
- **不做 phet 项目 git reset**：保留 `df833b6 → 7c3c1e3` 的错误痕迹作为物证
- **status 回退**：`in_progress` → `proposal_needs_rework`，表明整份计划需要基于新证据重写
- **不立即拟新方案**：新方案应在"完整的 phet 电路模块架构调研"完成后再产出，避免二次踩坑
...
**命题 1**：`class CircuitElement` 在 `c:\workspace\phet\**\*.dart` 里有几处引用？

| 工具 | 查询 | 结果 |
|---|---|---|
| grep_search | 正则 `\bCircuitElement\b`，`searchDirectory=c:\workspace\phet` | 0 命中（仅 AI 工作区文档命中） |
| terminal | `findstr /S /M /I "CircuitElement" c:\workspace\phet\*.dart` | 0 命中 |

**命题 2**：`battery.dart` 文件被 import 的位置？

| 工具 | 查询 | 结果 |
|---|---|---|
| grep_search | 正则 `battery\.dart`，`searchDirectory=c:\workspace\phet` | 0 命中 |
| terminal | `findstr /S /M /I "battery.dart" c:\workspace\phet\lib\*.dart c:\workspace\phet\test\*.dart` | 0 命中 |

**命题 3**：`vertex.dart` 文件被 import 的位置？

| 工具 | 查询 | 结果 |
|---|---|---|
| grep_search | 正则 `vertex\.dart`，`searchDirectory=c:\workspace\phet` | 0 命中 |
| terminal | `findstr /S /M /I "vertex.dart" c:\workspace\phet\lib\*.dart c:\workspace\phet\test\*.dart` | 0 命中 |

**命题 4**（反证）：`circuit_screen.dart` 的 import 段实际引用了哪些 `models/` 文件？

```dart
// c:\workspace\phet\lib\screens\circuit_screen.dart:5-7 (Get-Content 前 30 行实证)
...
```dart
// c:\workspace\phet\lib\widgets\optics_scene.dart:6-7 (Get-Content 前 20 行实证)
...
- `c:\workspace\phet\lib\models\battery.dart`
- `c:\workspace\phet\lib\models\circuit_element.dart`
- `c:\workspace\phet\lib\models\vertex.dart`
...
以上文档订正**在本次 Loop2 内一并完成**（都是 AI 工作区文档，零风险）；phet 项目源码的清理仍需用户明确 y/n 授权后执行。

---

## 1. 定位

本文档提出 phet 项目在**做任何配置化 / AI 生成改造之前**应该先执行的**架构基线治理**。

它**独立于** AI 生成目标存在——即使未来放弃 AI 生成，本文档提出的改动仍是值得做的技术债偿还。

## 2. 动机

`context/project/phet/architecture/design-patterns.md:126` 已诚实标注："配置化体系目前仅光学模块接入；电路模块是枚举驱动；力与运动完全硬编码。"
...
- `lib/models/optics_state.dart`（3.24 KB · 121 行）——知识库标注"@immutable，备用旧版"（`context/project/phet/systems/module-index.md:20`）
...
phet 项目曾尝试将 PhET CCK（Circuit Construction Kit）JavaScript 版本 100% 移植到 Dart，但**只完成了基类定义、未接入 CircuitScreen 的运行时**：
...
**关键验证点**：
- 业务电路代码使用的 `Vertex` 类是 `circuit_state.dart` 里的内嵌版本（[circuit_state.dart:34-47](c:\workspace\phet\lib\models\circuit_state.dart)）
...
**曾用的错误实证**（`Get-ChildItem lib/models -Filter *.dart | ForEach { grep "models/$name" lib/**/*.dart }`——**grep 作用域仅覆盖 `lib/`，未覆盖 `test/`；且模式只匹配 `models/xxx` 局部路径，漏 `package:geometric_optics/models/xxx.dart` 全包路径**）：
...
**关联的 phet git 历史**：`c2d47c2`（baseline） → `df833b6`（错误删除） → `7c3c1e3`（revert 回退，功能等价于 c2d47c2）

#### 3.1.b 本节的正确形态待定

后续如决定重启死代码清理，须先完成"phet 电路模块完整架构调研"（准确区分 lib/models/ 里哪些类是活/死代码），再据此重新拟定清理方案。可参考的**确定 zero-ref 类**（截至 2026-07-20 下午调研）：
...
- `context/project/phet/systems/module-index.md:20-22` 删除 5 个死代码文件的表格行
- `context/project/phet/systems/optics-module.md:60-68` 移除 `OpticsState`（备用旧版）/ `OpticsSolver`（备用旧版）章节
- 其余 phet 知识库文档 `grep -r "circuit_element\.dart\|battery\.dart\|vertex\.dart\|models/optics_state\|models/optics_solver\|models/circuit_"` 逐一处理
...
- `context/project/phet/systems/module-index.md:20-22` 删除 5 个死代码文件的表格行
- `context/project/phet/systems/optics-module.md:60-68` 移除 `OpticsState`（备用旧版）/ `OpticsSolver`（备用旧版）章节
- 其余 phet 知识库文档 `grep -r "circuit_element\.dart\|battery\.dart\|vertex\.dart\|models/optics_state\|models/optics_solver"` 逐一处理

**回退方式**：`c:\workspace\phet` 是 git 仓库（上轮已确认 master 分支）；但 `lib/` 目录 untracked——**建议在删除前先建立 baseline**：
```bash
cd c:\workspace\phet
...
- [ ] `context/project/phet/systems/module-index.md` 和 `optics-module.md` 同步更新（无"指向已删除代码"的坏引用）
...
- **新增依赖**：整份 refactor-baseline 重启前必须先完成"phet 电路模块完整架构调研"（详见 §0 教训与勘误）

## 7. 立需求建议

如你决定推进本方案，可考虑：

- **粒度 1**：`req-phet-refactor-baseline`（一个需求覆盖 3.1 + 3.2；3.3 视方案 B 决策再定）
- **粒度 2**：`req-phet-dead-code-batch2`（仅 3.1）+ `req-phet-circuit-module-dir`（仅 3.2）分开
- **SOP**：agile-vibe（含 PM/TL/Dev/Reviewer/Closer 4 阶段），因为涉及跨文件搬移 + 知识库联动

## 变更历史

> 2026-07-20：初版。基于 phet AI 生成就绪度调研（方案 B 前置工作）产出。作者: AI 调研会话。触发原因：调研过程中新识别的 problem C（`circuit_element.dart` + `battery.dart` + `vertex.dart` 死代码链）。
...
> - 关联 commit：phet 项目 `chore: remove dead lib/models/`（在 baseline `c2d47c2` 之后）
...
> - 关联 phet commit：`df833b6`（错误删除） → `7c3c1e3`（revert 回退）；两个 commit 均保留在 git 历史作为教训物证，不做 `git reset --hard` 抹除
...
> - 关联证据：`c:\workspace\phet\lib\screens\circuit_screen.dart:5-7` import 段（不含 3 个死文件） + `c:\workspace\phet\lib\widgets\optics_scene.dart:6-7` import 段（含 optics_state/optics_solver，证明其是活代码）
> - 未来动作：本 Loop2 一并订正 module-index.md + circuit-module.md（AI 工作区文档，零风险）；phet 源码清理待用户 y/n 授权
