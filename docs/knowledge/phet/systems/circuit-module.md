...
> 变更: 2026-07-17 删除 `CircuitSolverV2`（428 行 MNA 草稿）与 `ElectronAnimator`（100 行电子动画）两个未接入主流程的实验性组件，详见 `requirements/req-phet-dead-code-cleanup/`。
...
> ⚠️ **本节列出的 3 个文件在 `c:\workspace\phet` 全项目 zero-ref，是历史移植遗留的死代码。已通过 grep + terminal findstr 双工具交叉实证**。详见 [architecture/refactor-baseline-plan.md §0.6](../architecture/refactor-baseline-plan.md)。
