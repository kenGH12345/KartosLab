...
> 分析日期：2026-07-22 · Java 蓝本：`<PHET_JAVA_ROOT>/simulations-java/common/piccolo-kartos/src/.../nodes/slider/` + `nodes/radiobuttonstrip/`
...
### 2.1 核心控件：`KratosSlider`

```dart
/// Kratos 风格滑动条——min/max 区间内拖拽取 value。
///
...
/// )
/// ```
class KratosSlider extends StatelessWidget {
  // ── 必需参数 ──
...
### 2.2 滑动条替代控件：`KratosNumberField`

```dart
/// Kratos 风格数值输入框——对应 Java 版 GraphControlTextBox。
///
/// 用于"精确输入数值"场景（如 moving-man 的位置输入）。
class KratosNumberField extends StatelessWidget {
  final String label;
  final String? unit;
  final double value;
  final double min;
...
### 2.3 单选组：`KratosRadioGroup<T>`

```dart
/// Kratos 风格单选按钮组——对应 Java RadioButtonStripControlPanelNode。
///
/// 用法：
/// ```dart
...
/// )
/// ```
class KratosRadioGroup<T> extends StatelessWidget {
...
### 2.4 下拉选择：`KratosComboBox<T>`

```dart
/// Kratos 风格下拉选择——对应 Java ComboBoxNode / PhetPComboBox。
///
/// 用于选项较多时替代 RadioGroup。
class KratosComboBox<T> extends StatelessWidget {
  final String? label;
  final List<T> items;
...
///     KratosSlider(label: '折射率', min: 1.0, max: 2.5, value: n, onChanged: ...),
///     KratosRadioGroup<String>(label: '类型', items: ['凸透镜', '凹透镜'], value: type, onChanged: ...),
///   ],
/// )
/// ```
...
Flutter kratos 的 `§C2 元件规格来源` 硬约束要求控件参数从 scenario JSON 读取。`PropertyControlPanel` 提供工厂方法直接接入：
...
static PropertyControlPanel fromScenarioParams(
  ...
  // "slider" → KratosSlider
  // "radio" → KratosRadioGroup
  // "combo" → KratosComboBox
  // "number" → KratosNumberField
}
```

## 三、Java ↔ Dart 概念映射表

| Java (`SliderNode` / `VSliderNode` / `HSliderNode`) | Dart (`KratosSlider`) | 说明 |
|---|---|---|
...
| `VSliderNode` (垂直) | `KratosSlider(direction: Axis.vertical)` | |
| `HSliderNode` (水平) | `KratosSlider(direction: Axis.horizontal)`（默认） | |
...
| Java (`RadioButtonStrip` / `ComboBoxNode`) | Dart (`KratosRadioGroup` / `KratosComboBox`) | 说明 |
|---|---|---|
| `RadioButtonStripControlPanelNode` | `KratosRadioGroup<T>` | 单选组 |
| `ToggleButtonNode` (单选) | `ChoiceChip` / `SegmentedButton` | Flutter 原生替代 |
| `ComboBoxNode` / `PhetPComboBox` | `KratosComboBox<T>` | 下拉选择 |
...
| 控件 | MVP | 理由 |
|---|---|---|
| `KratosSlider` (水平) | ✅ | 最常用——几乎所有 sim 都会用 |
| `KratosSlider.vertical` (垂直) | ✅ | 温度/水位等竖直场景 |
| `KratosRadioGroup<T>` | ✅ | 模式切换（透镜类型、图表选择等） |
| `KratosComboBox<T>` | ✅ | 选项多时替代 RadioGroup |
| `PropertyControlPanel` | ✅ | 聚合容器——当前 Screen 的 `_RightPanel` 可替换 |
| `fromScenarioParams` 工厂 | ✅ | §C2 合规——所有参数从 JSON 来 |
| `KratosNumberField` | ❌ 可延后 | moving-man 精确输入才需要 |
| 垂直滑动条 `Axis.vertical` | ❌ 可延后 | 少数 sim 用 |
| 刻度标签 `tickLabels` | ❌ 可延后 | 视觉优化 |

## 五、与现有 Flutter kratos 代码的对齐

Flutter kratos 现状：每个 Screen 的 `_RightPanel` 手写 `Slider` + `TextField`（如 `optics_screen.dart` 光学模块右侧控制面板）。
...
// AFTER（迁移后 · 用 KratosSlider + 自动生成标签）
KratosSlider(
  label: '焦距', unit: 'cm',
  min: 5, max: 50, step: 1,
  value: lens.focalLength,
...
├── controls/
│   ├── kratos_slider.dart             # KratosSlider (水平/垂直)
│   ├── kratos_radio_group.dart          # KratosRadioGroup<T>
│   ├── kratos_combo_box.dart            # KratosComboBox<T>
│   └── kratos_number_field.dart         # KratosNumberField (可延后)
└── chart/
...
| `RadioButtonStripControlPanelNode.java` | piccolo-kartos 下 | 单选组：ToggleButtonNode[] 互斥 |
| `ComboBoxNode.java` | piccolo-kartos 下 | 下拉选择：PhetPComboBox(JComboBox 包装) |
| Flutter kratos 侧 §C2 | `architecture/design-patterns.md` | 元件规格来源硬约束（defaultParams/valueMin/valueMax/valueStep 从 JSON 来） |

---

> **下一步**：
> 1. 如批准本草案 → 在 `c:\workspace\kratos\lib\common\controls\` 创建 4 个文件 + `widgets/property_control_panel.dart`
> 2. optics 模块作为首个迁移目标——用 `KratosSlider` 替代 `_RightPanel` 里的原装 `Slider`
