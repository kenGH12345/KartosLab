# kratos-java-simulations 知识库 · 决策与局限记录
...
- **决定**：新建 `docs/knowledge/kkartoss-java-simulations/` 独立知识库，与 `docs/knowledge/kartosos/`（Flutter 复刻）**并列**
- **理由**：Java 版是**只读参考蓝本**，与 Flutter 复刻不属于同一"实施项目"；用户明确选了关系 c（并列 + 交叉引用）
- **反面选项**：也可以放到 `docs/knowledge/kkartoss/reference/java-simulations/`（作为 Flutter 项目的子目录），但会混淆"我们要维护的项目"和"我们要参考的蓝本"

### 决策 2：路径占位符约定

- **决定**：所有文档中出现 Java 源码绝对路径时，用 `<PHET_JAVA_ROOT>` 占位
- **对应**：当前调研时的实际路径是 `C:\workspace\kartosTrunk\kartosTrunk`
...
- **理由**：Value 权重最高（因为 kartosos 复刻主要目标是"教学价值")；Fit 与 复杂度并列（技术风险控制）；依赖最低（因为 🔴 3D 已经在候选池外）
- **可调整**：若 Flutter kartosos 团队优先级不同（例如"快速出 PR"），可提高复杂度权重；若"追求教学影响"，可提高 Value 权重
...
- 基于 PhET 官网 [kartos.colorado.edu/en/simulations/browse](https://kartos.colorado.edu/en/simulations/browse) 的公开热度经验判断
- 未做严格的下载量 / 教师使用率统计
...
- 基于本次调研对 Flutter kratos 3 模块（[kratos/systems/*.md](../kratos/systems/)）的阅读
- 未做真实"移植 PoC" 验证
...
### 局限 5：📢 用户明确纠正的边界
- **知识库存在性**：Loop 1 之前调研过程中我错误声称"没有 kartosos 项目的知识库"，用户提醒后才发现 `docs/knowledgekartostos/` 早已存在完整知识库（含 INDEX / systems / architecture）
...
- **[待办 C]** 依赖库解剖：本次未展开 24 个 common 库的功能划分（如 kartoscommon / piccolo-kartos 各自负责什么）· 若要复刻大量 sim 建议补一份 [`common-libraries.md`]
...
- **[待办 F]** 复刻覆盖率仪表盘：随着 Flutter kartosos 复刻推进，建议在 [existing-flutter-map.md] 里维护"已完成 / 进行中 / 待办"三色标记

## 四、维护提示

### 何时应更新本知识库
- ✅ Flutter kartosos 完成一个新模块复刻 → 更新 [existing-flutter-map.md]（补该 sim 的 Java 蓝本映射）
...
### 何时不用改本知识库
- Flutter kartosos 内部重构（不涉及 Java 蓝本映射）
- Flutter kratos 的 AI 生成工具链演进
- 只涉及 [kratos/](../kratos/) 侧的文档
...
- Flutter 侧参考：`docs/knowledge/kkartoss/systems/module-index.md` + `<module>-module.md`
