...
> **蓝本**：`c:\workspace\kartosTrunk\kartosTrunk\simulations-java\simulations\sound\` · 111 Java 文件
...
> **数据源**：`PHET_JAVA_ROOT/simulations-java/simulations/sound/src/edu/colorado/kartos/sound/**/*.java`
...
| AWT `JSlider` / `JButton` / `JRadioButton` | ❌ 用 Flutter `KratosSlider` / `KratosRadioGroup`（L0 复用） |
...
│   └── widgets/
│       ├── sound_control_panel.dart     (频率+振幅滑块 · 复用 L0 KratosSlider)
...
1. **Role** · "You are a sound/acoustics experiment designer for kratos Flutter port"
...
| 组件 | 来源路径 | 必用/可选 | 理由 |
|---|---|---|---|
| KratosSlider | `lib/common/controlkartosatos_slider.dart` | 必用 | 频率/振幅/墙角度/墙位置滑块 |
| KratosRadioGroup | `lib/common/controlkartosatos_radio_group.dart` | 必用 | 子屏切换 / 脉冲/连续模式切换 |
...
| KratosComboBox | `lib/common/controlkartosatos_combo_box.dart` | 可选 | 若暴露 wavefrontType 切换 |
...
- 本 sim 涉及的 L0 层：KratosSlider / KratosRadioGroup / PropertyControlPanel / TimeControlBar / SimulationClock
