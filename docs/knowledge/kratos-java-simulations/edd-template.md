...
> **版本历史**：v1.0（memory 2q03lm2g §4.6.7 原始 8 章 · EGPLab 沉淀）→ v2.0（2026-07-24 · kratos-java-simulations 项目补 §9-§12 · 闭合四原则覆盖）
...
> **参照现有**：`c:\workspace\kratos\assets\scenarios\{circuit,forces}\` + `docs/prompts/*.md` + `schemas/*.schema.json`
...
- [ ] kratos-java-simulations 项目进入稳定期（不再有新 sim 立项）
